...
const emailSender = async (req, res) => {
  ...
  if (req.body.user || req.body.employee) {
    ...
    if (req.body.user) {
      ...
      mailOptions.text = `${req.body.fullName} ,your account has being created successfully, to confirm email click on this link ${process.env.FrontEndBaseUrl}/verify-email?token=${await jwtForSignUp(
        req.body.user._id,
        verificationCode
      )}`;
    } else {
      mailOptions.text = `${req.body.fullName} your account has being successfully created, to confirm email and set password click this link ${
